    ...
    def __init__(self, input_dim, encoder_hidden_dims, decoder_hidden_dims):
        ...
        for i in range(len(encoder_hidden_dims)):
            if i == 0:
                ...
                encoder_layers.append(nn.Conv2d(input_dim, encoder_hidden_dims[i], 1, 1))
            else:
                ...
                encoder_layers.append(nn.ReLU())
                encoder_layers.append(nn.Conv2d(encoder_hidden_dims[i - 1], encoder_hidden_dims[i], 1, 1))
        ...
        for i in range(len(decoder_hidden_dims)):
            if i == 0:
                ...
                decoder_layers.append(nn.Conv2d(encoder_hidden_dims[-1], decoder_hidden_dims[i], 1, 1))
            else:
                # decoder_layers.append(nn.ReLU())
                # decoder_layers.append(nn.Linear(decoder_hidden_dims[i - 1], decoder_hidden_dims[i]))
                decoder_layers.append(nn.ReLU())
                decoder_layers.append(nn.Conv2d(decoder_hidden_dims[i - 1], decoder_hidden_dims[i], 1, 1))
        decoder_layers.append(nn.Conv2d(decoder_hidden_dims[i], input_dim, 1, 1))
