...
from internal.utils.colmap import qvec2rotmat, read_model
...
parser.add_argument("--invdepth_thresh", type=float, default=0.05)
args = parser.parse_args()

if args.depth_dir is None:
    args.depth_dir = os.path.join(args.dataset_dir, "extra", "estimated_invdepth")
if args.output is None:
    args.output = os.path.join(args.dataset_dir, "extra", "estimated_depth_scales.json")
...
points3d_error_ordered = np.zeros(
    [
        pts_indices.max() + 1,
    ]
)
...
def get_scales(key, cameras, images, points3d_ordered, points3d_error_ordered, args):
    ...
    if valid.sum() > 10 and (invcolmapdepth.max() - invcolmapdepth.min()) > 1e-3:
        ...
        invmonodepth = cv2.remap(
            invmonodepthmap, maps[..., 0], maps[..., 1], interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REPLICATE
        )[..., 0]
    ...
    # invmonodepthmap_aligned = invmonodepthmap * scale + offset
    binary_mask = ((invmonodepthmap > args.invdepth_thresh) * 255).astype(np.uint8)
    dst_path = f"{args.dataset_dir}/extra/masks_from_depth/{image_meta.name + '.png'}"
