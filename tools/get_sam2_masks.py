...
from utils.common import AsyncImageReader, AsyncImageSaver, AsyncTensorSaver
...
parser.add_argument("--sam_ckpt", "-c", type=str, default="checkpoints/sam2/sam2.1_hiera_large.pt")
parser.add_argument("--sam_cfg", type=str, default="configs/sam2.1/sam2.1_hiera_l.yaml")
...
sam2_model = build_sam2(args.sam_cfg, args.sam_ckpt, device=MODEL_DEVICE)
        ...
        for _ in t:
            ...
            for m in masks:
                ...
                if args.preview is True:
                    ...
                    transparency = (torch.tensor(m["segmentation"], dtype=torch.float, device=img_tensor.device) * 0.5)[
                        ..., None
                    ]
                    img_tensor = img_tensor * (1 - transparency) + transparency * color_mask

                    image_saver.save(
                        img_tensor.to(torch.uint8).cpu().numpy(), os.path.join(mask_preview_dir, f"{image_name}.png")
                    )
