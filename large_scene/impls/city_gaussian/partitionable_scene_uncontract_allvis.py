...
@dataclass
class UncontractAllVisCitySceneConfig(PartitionableSceneConfig):
...
@dataclass
class UncontractAllVisCityScene(CityScene):
    scene_config: UncontractAllVisCitySceneConfig = field(default_factory=lambda: UncontractAllVisCitySceneConfig())
    ...
    def calculate_camera_visibilities(self, coarse_model, renderer, cameras, device="cpu", bg_color=...):
        ...
        for partition_idx in range(len(self.partition_coordinates)):
            positions = coarse_model.get_xyz[self.gaussians_in_partitions[partition_idx, :]]
            for camera_idx, camera in enumerate(cameras):
                full_projection = camera.full_projection.to(device)
