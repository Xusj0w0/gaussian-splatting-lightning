# fmt: off
# isort: skip_file
# modified from utils/trained_partition_utils.py
...
def get_trained_partitions(
    ...
    partition_training = ModifiedPartitionTraining(
        ModifiedPartitionTrainingConfig(
    ...
    for partition_idx in tqdm(trainable_partition_idx_list, desc="Searching checkpoints"):
        ...
        assert os.path.exists(
            os.path.join(
                partition_training.project_output_dir,
                partition_training.get_partition_trained_step_filename(partition_idx),
            )
        ...
        assert ckpt_file.endswith(".ckpt"), "checkpoint of partition #{} ({}) can not be found in {}".format(
            partition_idx, partition_id_str, partition_training.project_output_dir
        )
    ...
    orientation_transformation = (
        partition_training.scene["extra_data"]["rotation_transform"]
        if partition_training.scene["extra_data"] is not None
        else None
    )
...
def get_partition_gaussian_mask(
    ...
    if orientation_transform is not None:
        means = means @ orientation_transform[:3, :3].T + orientation_transform[:3, -1]
...
def split_partition_gaussians(
    ckpt: dict, partition_bounding_box: MinMaxBoundingBox, orientation_transform: torch.Tensor = None
) -> tuple[
    ...
    is_in_partition = get_partition_gaussian_mask(
        model.get_means(), partition_bounding_box, orientation_transform=orientation_transform
    )
