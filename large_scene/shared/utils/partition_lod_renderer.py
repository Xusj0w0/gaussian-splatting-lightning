...
from internal.renderers.gsplat_v1_renderer import GSplatV1Renderer, GSplatV1, spherical_harmonics
...
class PartitionLoDRendererModule(Renderer):
    ...
    def setup(self, stage: str, *args: Any, **kwargs: Any) -> Any:
        ...
        self.translation = self.orientation_transform[:3, -1].to(device=device)
        ...
        from .partition_trained_utils import ModifiedPartitionTraining, ModifiedPartitionTrainingConfig
        ...
        ], dim=1) - self.translation) @ self.orientation_transform.T  # [N_partitions, 8, 3], in world space
    ...
    def get_partition_distances(self, p: torch.Tensor):
        p = (p @ self.orientation_transform + self.translation)[:2]
    ...
    def setup_labels(self):
        ...
        for idx, (id, xy, size) in enumerate(self.renderer.partition_coordinates):
            self.label_updaters.append(get_label_updater(
                ...
                position=((torch.concat([xy + 0.5 * self.renderer.default_partition_size, torch.tensor([0.])]).cuda() - self.renderer.translation) @ self.renderer.orientation_transform.T).cpu().numpy(),
            ))
