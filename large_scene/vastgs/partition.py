...
class VastGSPartitiongConfig:
    ...
    def configure_argparser(parser: ArgumentParser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help='Partition info dir (containing scene configs) will under the "output path".',
        )
        parser.add_argument(
            "--partition_dim",
            required=True,
...
class VastGSPartitioning:
    def __init__(self, config: VastGSPartitiongConfig):
        ...
        self.output_path = osp.join(self.config.output_path, self.scene.build_output_dirname())
    ...
    def read_scene(self):
        ...
        dataparser: ColmapDataParser = dataparser_config.instantiate(
            path=self.dataset_path, output_path=self.output_path, global_rank=0
        )
