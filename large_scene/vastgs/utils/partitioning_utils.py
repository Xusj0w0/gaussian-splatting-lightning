...
class VastGSScene(PartitionableScene):
    ...
    def plot_partition_assigned_cameras(
        self,
        ax,
        partition_idx: int,
        point_xyzs: torch.Tensor = None,
        point_rgbs: torch.Tensor = None,
        point_size: float = 0.1,
        point_sparsify: int = 8,
    ):
        ...
        location_base_assignment_bounding_box_sizes = (
            location_base_assignment_bounding_boxes.max - location_base_assignment_bounding_boxes.min
        )
        ...
        visibility_base_assignment_bounding_box_size = (
            visibility_base_assignment_bounding_boxes.max - visibility_base_assignment_bounding_boxes.min
        )
        ...
        if point_xyzs is not None:
            ax.scatter(
                point_xyzs[::point_sparsify, 0],
                point_xyzs[::point_sparsify, 1],
                c=point_rgbs[::point_sparsify] / 255.0,
                s=point_size,
            )
        ...
        ax.scatter(
            self.camera_centers[location_based_assignment.numpy(), 0],
            self.camera_centers[location_based_assignment.numpy(), 1],
            s=0.2,
            c="blue",
        )
        ax.scatter(
            self.camera_centers[visibility_based_assignment.numpy(), 0],
            self.camera_centers[visibility_based_assignment.numpy(), 1],
            s=0.2,
            c="red",
        )
        ...
        ax.annotate(
            ...
            xy=self.partition_coordinates.xy[partition_idx],
            color="orange",
        )
