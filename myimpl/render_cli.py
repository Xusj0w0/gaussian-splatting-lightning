...
def initializer_viewer_renderer(
    ...
    if len(args.model_paths) == 1 and args.model_paths[0].endswith(".yaml"):
        ...
        load_device = (
            torch.device("cuda") if len(model_paths) == 1 or enable_transform is False else torch.device("cpu")
        )
...
def process_image_queue(
    image_save_batch: int, image_queue: queue.Queue, video_writer: mediapy.VideoWriter, frame_output_path: str
):
    class MockQueue:
    ...
    image_to_video_thread = threading.Thread(
        target=process_image_to_video_queue, args=(image_to_video_queue, video_writer)
    )
