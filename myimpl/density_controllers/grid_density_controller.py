...
class GridGaussianDensityControllerImpl(VanillaDensityControllerImpl):
    ...
    def _densify_and_prune(self, gaussian_model: GridGaussianModel, optimizers: List):
        ...
        if isinstance(gaussian_model, LoDGridGaussianModel):
            self._densify_multilevel_anchors(
                grads=grads_norm, primitive_mask=primitive_mask, gaussian_model=gaussian_model, optimizers=optimizers
            )
        else:
    ...
    def _densify_anchors(self, grads, primitive_mask, gaussian_model: GridGaussianModel, optimizers):
        ...
        filtered_res: CandidateAnchors = GridDensityContollerUtils.filter_primitives(
        ...
        if filtered_res.n_anchors > 0:
            ...
            new_properties = OptimizerManipulation.cat_tensors_to_properties(property_dict, gaussian_model, optimizers)
            gaussian_model.properties = new_properties

    def _densify_multilevel_anchors(self, grads, primitive_mask, gaussian_model: LoDGridGaussianModel, optimizers):
        ...
        for cur_level in range(gaussian_model._max_level):
            self._densify_anchor_per_level(
    ...
    def _densify_anchor_per_level(
        ...
        filtered_res: CandidateAnchors = GridDensityContollerUtils.filter_lod_primitives(
            gaussian_model=gaussian_model,
            all_primitives=all_primitives,
            grad_mask=grad_mask,
            res_level=cur_level,
        ...
        filtered_res_ds: CandidateAnchors = GridDensityContollerUtils.filter_lod_primitives(
            gaussian_model=gaussian_model,
            all_primitives=all_primitives,
        ...
        if filtered_res.n_anchors > 0:
            ...
            new_properties = OptimizerManipulation.cat_tensors_to_properties(property_dict, gaussian_model, optimizers)
            gaussian_model.properties = new_properties
            # self._anchor_denom = torch.cat(
            #     [self._anchor_denom, self._anchor_denom.new_zeros((filtered_res.n_anchors, 1))], dim=0
            # )
            # self._anchor_opacity_accum = torch.cat(
            #     [self._anchor_opacity_accum, self._anchor_opacity_accum.new_zeros((filtered_res.n_anchors, 1))], dim=0
            # )
        if filtered_res_ds.n_anchors > 0:
            ...
            gaussian_model.properties = new_properties
            # self._anchor_denom = torch.cat(
            #     [self._anchor_denom, self._anchor_denom.new_zeros((filtered_res_ds.n_anchors, 1))], dim=0
            # )
            # self._anchor_opacity_accum = torch.cat(
            #     [self._anchor_opacity_accum, self._anchor_opacity_accum.new_zeros((filtered_res_ds.n_anchors, 1))],
            #     dim=0,
            # )

    def _prune_anchors(self, keep_mask, gaussian_model: GridGaussianModel, optimizers):
        new_properties = OptimizerManipulation.prune_properties(keep_mask, gaussian_model, optimizers)
        gaussian_model.properties = new_properties
...
class GridDensityContollerUtils:
    @staticmethod
    ...
    def filter_primitives(
        ...
        if overlap:
            ...
            keep_mask = GridDensityContollerUtils.filter_exsiting_grids(candidate_grids, existing_grids)
    ...
    def filter_lod_primitives(
        ...
        if not is_next_level or (gaussian_model.activate_level >= gaussian_model.max_level and res_level < gaussian_model.max_level):
            if candidate_grids.shape[0] > 0:
                ...
                weed_keep_mask = GridGaussianUtils.weed_out_mask_by_level(
                    candidate_anchors,
                    candidate_levels,
