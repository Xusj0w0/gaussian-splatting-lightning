...
from myimpl.utils.octree_utils import OctreeUtils


@dataclass
class OctreeDensityController(VanillaDensityController):
    ...
    def instantiate(self, *args, **kwargs):
        return OctreeDensityControllerImpl(self)


class OctreeDensityControllerImpl(VanillaDensityControllerImpl):
    config: OctreeDensityController

    def __init__(self, config, *args, **kwargs):
        super().__init__(config, *args, **kwargs)

        density_states = ["_offset_gradient_accum", "_offset_denom", "_opacity_accum", "_denom"]
    ...
    def setup(self, stage: str, pl_module: LightningModule) -> None:
        ...
        if stage == "fit":
            ...
            self._init_octree_state(n_anchors, n_offsets, device)
    ...
    def _init_octree_state(self, n_anchors: int, n_offsets: int, device):
    ...
    def _densify_and_prune(self, gaussian_model: OctreeGaussianModel, optimizers: List):
    ...
    def _densify_anchor_single_level(
        ...
        """
        Steps:
            1. select offsets that offset_gradient > thresh
            2. voxelize offsets and get unique grids
            3. remove existing grids (according to whether `overlap`), and further filter by levels predicted by cameras
            4. cat new anchors to optimizers
        Intermediate variables:
            - **candidate_mask**: bool [N_anchors * N_offsets, ]. Indicating primitives that satisfy grad norm thresh.
            - **inverse_indices**: int [N_uni, ]. Indicating the unique index in grad filtered primitives.
            - **remove_duplicates**: bool [N_uni, ]. Indicating whether the anchor is existing and is visible.
            - **num_new_anchors**: int. num_new_anchors = remove_duplicates.sum()
        """
        ...
        candidate_mask_ds = grads >= ds_threshold
        ...
        if n_anchors_diff > 0:
            ...
            candidate_mask_ds = torch.cat(
                [candidate_mask_ds, candidate_mask_ds.new_zeros((n_anchors_diff * n_offsets,))], dim=0
            )
        ...
        candidate_mask_ds = torch.logical_and(candidate_mask_ds, level_mask_repeat)
        ...
        candidate_anchor, candidate_level, filter_mask = self._filter_candidate_anchors(
        ...
        selected_xyz_ds = all_xyz.view(-1, 3)[candidate_mask_ds]
        ...
        if (
            ...
            candidate_anchor_ds, candidate_level_ds, filter_mask_ds = self._filter_candidate_anchors(
                grid_coords_ds, selected_grid_coords_unique_ds, gaussian_model, ds_size, cur_level + 1
            )
        else:
            filter_mask_ds = level_ds_mask.new_zeros((selected_grid_coords_unique_ds.shape[0],))
            candidate_anchor_ds = selected_grid_coords_unique_ds.new_zeros((0, 3))
            candidate_level_ds = levels.new_zeros((0,))

        if candidate_anchor.shape[0] > 0:
            self._cat_new_properties_to_optimizers(
                candidate_infos=(
                    candidate_anchor,
                    candidate_level,
                    candidate_mask,
        ...
        if candidate_anchor_ds.shape[0] > 0:
            self._cat_new_properties_to_optimizers(
                candidate_infos=(
                    candidate_anchor_ds,
                    candidate_level_ds,
    ...
    def _get_new_properties(
        self,
        candidate_infos: Tuple[torch.Tensor],
        gaussian_model: OctreeGaussianModel,
        cur_size: float,
    ):
        """
        Return:
            ...
            - **num_new_anchors**. int
        """
        anchor, level, *_ = candidate_infos
        num_new_anchors = anchor.shape[0]
        n_offsets = gaussian_model.n_offsets

        new_scales = gaussian_model.get_scalings.new_ones((num_new_anchors, 6)) * cur_size
        new_scales = gaussian_model.scale_inverse_activation(new_scales)
        new_offsets = gaussian_model.get_offsets.new_zeros((num_new_anchors, 1, 3)).repeat(1, n_offsets, 1)
        new_extra_levels = gaussian_model.get_extra_levels.new_zeros((num_new_anchors,))
    ...
    def _cat_new_properties_to_optimizers(
        self,
        candidate_infos: Tuple[torch.Tensor],
        gaussian_model: OctreeGaussianModel,
        cur_size: float,
        ...
        new_properties, num_new_anchors = self._get_new_properties(
            candidate_infos, gaussian_model=gaussian_model, cur_size=cur_size
        )
        new_parameters = DensificationUtils.cat_tensors_to_properties(new_properties, gaussian_model, optimizers)
    ...
    def _densify_anchors(self, grads, offset_mask, gaussian_model: OctreeGaussianModel, optimizers):
        ...
        for cur_level in range(gaussian_model._max_level):
            self._densify_anchor_single_level(
    ...
    def _prune_anchors(self, denom_mask, opacity_mask, gaussian_model: OctreeGaussianModel, optimizers):
        n_offsets = gaussian_model.n_offsets

        keep = ~torch.logical_and(denom_mask, opacity_mask)
        new_parameters = DensificationUtils.prune_properties(keep, gaussian_model, optimizers)
        gaussian_model.properties = new_parameters
    ...
    def on_load_checkpoint(self, module, checkpoint):
        ...
        assert (
            "_denom" in density_state_dict and "_offset_denom" in density_state_dict
        ), "Density controller states not found in checkpoint"
        n_anchors = density_state_dict["_denom"].shape[0]
        n_offsets = density_state_dict["_offset_denom"].shape[0] // n_anchors
        self._init_octree_state(n_anchors, n_offsets, device="cpu")
